  ...
  for (int endc = 1; endc <= 2; endc++) {
    for (int stat = 1; stat <= 4; stat++) {
	  ...
	  if (debug) {
	    ...
	    if (stat==1 && ring==4 ) 
	    	std::cout <<"CompareALCTs "<< strstrm.str()<< std::endl;
	  }
	  ...
	  for (int i = 0; i < ndata; i++) {
	    ...
	    stubs_comparison[0].bx_data = alctV_data[i].getBX();
	    stubs_comparison[0].fullbx_data = alctV_data[i].getFullBX();
	    GlobalPoint gp_alct_data(getGlobalPosition(detid.rawId(), data_wiregroup, 10));
	    ...
	    for (int j=0; j<nemul; j++){
	      ...
	      GlobalPoint gp_alct_emul(getGlobalPosition(detid.rawId(), emul_wiregroup, 10));
    ...
    //cerr
	  ...
	  for (int i = ndata; i<nemul; i++){
	      ...
	      GlobalPoint gp_alct_emul(getGlobalPosition(detid.rawId(), alctV_emul[i].getKeyWG(), 10));
	      stubs_comparison[0].eta_emul = gp_alct_emul.eta();
	      // Emulator BX re-calculated for comparison with BX in the data.
	      if (isTMB07)
		emul_corr_bx = emul_bx - rawhit_tbin_offset + register_delay;
  ...
  for (int endc = 1; endc <= 2; endc++) {
    for (int stat = 1; stat <= 4; stat++) {
	  ...
	  for (pd = clctV_data.begin(); pd != clctV_data.end(); pd++) {
	    ...
	    GlobalPoint gp_clct_emul(getGlobalPosition(detid.rawId(), testwg, (*pd).getKeyStrip()));
